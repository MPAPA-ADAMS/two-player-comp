...
export default function TournamentNavigation({
  ...
  const [statuses, setStatuses] = useState<Map<number, TournamentStatus>>(
    () => new Map(tournaments.map((tournament) => [tournament.id, tournament.status])),
  );

  useEffect(() => {
    const refresh = () => setStatuses(getEffectiveTournamentStatuses(tournaments));
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800 px-5 py-4">
        ...
        <p className="text-sm text-zinc-500">{tournaments.length} tournaments</p>
      </div>
...
function TournamentNavigationButton({ tournament, active }: TournamentNavigationButtonProps) {
  ...
  const content = (
    <>
      ...
      <p className={`mt-2 text-left text-[11px] font-bold uppercase tracking-wider ${active ? "text-zinc-800" : getStatusTextClass(tournament.status)}`}>
  ...
  return (
    <Link href={`/tournaments/${tournament.id}`} aria-current={active ? "page" : undefined} className={className}>
      {content}
    </Link>
  );
}
...
function StatusIndicator({ status }: { status: TournamentStatus }) {
  return <span aria-hidden="true" className={`h-2.5 w-2.5 rounded-full ${status === "COMPLETED" ? "bg-emerald-400" : status === "READY" ? "bg-amber-400" : status === "IN_PROGRESS" ? "bg-blue-400" : "bg-zinc-700"}`} />;
}

function getStatusLabel(status: TournamentStatus) {
  return { LOCKED: "Locked", READY: "Ready", IN_PROGRESS: "Live", COMPLETED: "Complete" }[status];
}

function getStatusTextClass(status: TournamentStatus) {
  return { LOCKED: "text-zinc-700", READY: "text-amber-400", IN_PROGRESS: "text-blue-400", COMPLETED: "text-emerald-400" }[status];
}
